...
import { getWorkspaceId, StorageKeys } from '../base/storageKeys'
import {
  deletePayload,
  isStorageAvailable,
  readIndex,
  ...
  try {
    for (const entry of entries) {
      ...
      if (localStorage.getItem(entry.key) !== null) {
        throw new Error(`Legacy storage key ${entry.key} was not removed`)
      }
      removed.push(entry)
    }
    return removed
...
function hasPersistentOpenPaths(
  ...
  try {
    const raw = localStorage.getItem(StorageKeys.lastOpenPaths(workspaceId))
    if (!raw) return false
    ...
    return (
      ...
      parsed.paths.length === expectedPaths.length &&
      parsed.paths.every((path, index) => path === expectedPaths[index]) &&
...
function cleanupV1TabState(workspaceId: string, clientId?: string): void {
...
export function migrateV1toV2(
  ...
  if (sources.length === 0) {
    ...
    if (originalIndex) {
      ...
      cleanupV1TabState(workspaceId, clientId)
      return -1
    }
  ...
  for (const path of legacy.order) {
    ...
    const currentEntry = baseIndex.entries[draftKey]
    if (currentEntry) {
