...
test.describe('Workflow persistence recovery regressions', () => {
  ...
  test('Does not save a startup-only workflow and deduplicates load failures', async ({
    ...
    // pagehide synchronously flushes pending workflow persistence. Keep
    // observing for longer than the persistence debounce window so a delayed
    // duplicate notification cannot slip past an immediately-satisfied locator
    // assertion.
    ...
    await expect
      .poll(
        async () => {
          duplicateObserved ||=
            (await comfyPage.toast.toastErrors.count()) !== 1
          return !duplicateObserved && Date.now() - observationStartedAt >= 750
        },
        { timeout: 1_000, intervals: [75] }
      )
      .toBe(true)
  })
